...
**11개 전부** 등록해야 한다. 전체 표(이름·설명·가격 포함)는
**`docs/iap_products_table.md`** 에 있다 — `iap.json` 에서 자동 생성하므로
항상 코드와 일치한다. 콘솔에는 그 표를 그대로 옮기면 된다.

> 2026-07-20: 젤리 팩 지급량을 조정했다. 기존에는 표기 보너스와 실제 가치가
> 어긋나 **특대(29,000원) 팩이 소형 대비 +3%** 에 불과했다(표기는 +25%).
> 지금은 0/10/20/31/42% 로 살수록 이득이 커지고 표기와 실제가 일치한다.
