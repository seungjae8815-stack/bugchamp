...
- **패키지 구조**(의존 방향 `app → core_battle/core_run → core_models`):
  ...
  - `app` — UI·저장·데이터·다국어.
- **검증 상태**: 순수패키지 테스트(core_models 34·core_battle 7·core_run 31) + 앱 65개 통과(총 137), 5개 패키지 analyze 무결, 데이터 정합성 통과(battle.json 포함).
...
- 채팅 바(placeholder), 랭킹(coming soon), Supabase 비동기 매칭·리더보드(Phase 4 예정).
