...
> 마지막 갱신: 2026-07-15 (**수동 배틀** + **부상/회복** + **PvP 보상 JSON화·스카우트 보드** + **리그/티어** + **시즌 리셋** + **아레나 폴리시** 반영. 그 전: PvP 아레나 + 오행 전투엔진).
...
- **패키지 구조**(의존 방향 `app → core_battle/core_run → core_models`):
  ...
  - `app` — UI·저장·데이터·다국어.
- **검증 상태**: 순수패키지 테스트(core_models 31·core_battle 7·core_run 28) + 앱 61개 통과(총 127), 5개 패키지 analyze 무결, 데이터 정합성 통과(battle.json 포함).
- **세이브 스키마**: `kSaveSchemaVersion = 14` (마이그레이션 v0→v14 전부 존재).
...
`kSaveSchemaVersion = 14`. 필드 추가 이력:
- v4 닉네임·버프 / v5 미션 / v6 장착 / v7 일일보상 / v8 선물+다음선물시각 / v9 클리어챕터 / v10 부화기 / v11 PvP 트로피 / v12 부상(injured) / v13 승급보상(claimedLeagues) / **v14 시즌(seasonStartedAt·seasonPeakTrophies)**.
...
1. **§6 위반(수치는 JSON)**: PvP 보상은 ✅ **`battle.json` 이관 완료**(2026-07-15, `BattleConfig`). 남은 하드코딩: **분해 보상=포텐셜**(save_controller `disassembleBug`) — 소소하나 정리 대상.
2. ~~**CLAUDE.md 드리프트**~~ ✅ **현행화 완료**(2026-07-15): §2.3 전투(공/방/회+오행+기력)·신규 시스템(부화기·돌파·PvP·부상·리그·시즌 등 §2.7)·브리딩 미구현·트랩 레거시·core_run/core_gathering 패키지·폴더구조 모두 반영. (트랩 채집 core_gathering **유지/삭제**는 여전히 사장님 결정 필요 — 현재는 세이브 호환용 잔존)
...
7. ~~**순서 ①②③ + 상생 미리보기** UI~~ ✅ **완료**(2026-07-15) — 슬롯 순서 배지 + 상생 연결·팀 시너지% 미리보기. 남은 여지: 슬롯 드래그로 순서 변경, 상성(克) 상대 대비 미리보기.
