...
> 마지막 갱신: **2026-09-15** — §00(2026-09-14~15, **사냥터 대개편 + 대회 2회차 준비**, 미배포)을 추가했다.
> 1.0.7 ~ 1.0.11(2026-08-27 ~ 09-12) 사이클은 이 문서에 따로 적지 않았다 — 출시 범위는
> `docs/_release_notes_1.0.*.txt`·`_appstore_whatsnew_1.0.*.txt` 와 git 로그가 원본이다.
>
> 그 전 2026-08-26 — §0(2026-08-17~25, **1.0.6 개발·출시 사이클**). 로컬에만 있던 67커밋을 `origin/main` 에 푸시 완료(`efc85fe..984d7f9`).
>
...
## 00. 2026-09-14 ~ 09-15 — 사냥터 대개편 + 대회 2회차 준비 (**미배포**)

마지막 출시는 **1.0.11+20260912**. ①은 main 에 커밋됨(`170a12f`), ②③은 09-15 작업분이다. 전부 스토어·서버에 안 나갔다.
다음 출시는 **1.0.12** 로 올린다(`pubspec.yaml` 은 아직 `1.0.11+20260928`).

### ① 사냥터·보스 대개편 (09-14, 설계 `docs/design_zones.md`)
- 스테이지 1000개 → **난이도마다 사냥터 10 + 최종 보스**. 사냥터 안은 평탄·무한, 100마리 잡으면 보스 도전.
  기존 유저는 새 앱 로드에서 전원 사냥터 1(`kZoneEpoch`, ⚠️ 서버 로드에서는 하지 않는다 — hotfix c1cec41).
- 보스 아트 44마리 × 6동작, 로드맵 세로 목록 + 점령 전 실루엣.
- 밸런스: 사냥터 표(도착 시점 / 머문 뒤 기준), 위협 적응형 복구, 캐릭터가 한 대를 전부 받음,
  장비 옵션 상한 하향(보스 DPS x33.7 → x6.1), 회차 전환 시 골드·일반 재료 초기화. CLAUDE.md §2.4·§2.7.
- 실기기 튜닝 진행 중(사장님 폰). 손잡이는 `run_config.json`(한 대 크기·처치 회복·곤충 몫).

### ② 09-15 실기기 피드백
- **곤충이 캐릭터보다 먼저 쳤다** → 몬스터마다 캐릭터 첫 타 뒤 슬롯 순서대로(0.08초 간격) 따라 친다.
  곤충 누산기는 기다리는 동안에도 쌓여 DPS 는 그대로(`play_screen.dart _playerStruckT`).
- **랭킹 진행도** `보통 2-001` → `보통 · 사냥터 3` + 전투력. 정렬 회차 → 사냥터 → 전투력
  (`profiles.power`, `_sql_20260915_rank_power.sql`). 전투력은 홈 상단과 같은 함수(`domain/combat_power.dart`).
- **보스 처치 미션 → 제련 100회(고정, 보상 젤리 1~3 그대로)**. 사냥터 구조에선 보스 미션이 안 차서
  순서대로 도는 미션 전체가 멈췄다. 제련은 화석(시간 비례)이 속도를 정해 목표를 키울 이유가 없다.
  jelly_sim 미션 3.0/일 그대로. ⚠️ 새 서버 + 옛 앱은 `boss` 미션 수령이 거절된다 — 서버·앱 배포 간격을 짧게.

### ③ 왕충 선발대회 1회차 종료 → 2회차 대기 (09-15)
- 뱃지 3종: 1위 `champion` · 2~10위 `finalist` · 참가 전원 `participant`(신설). 대표 뱃지 = 급 → 최근 회차.
- **채팅에 뱃지** — insert 트리거가 `profiles.badge` 에서 찍는다(`_sql_20260915_chat_badge.sql`).
- 서버: 뱃지 번호를 **그 유저가 뛴 회차**로(`roundNoOf` + `event.json → pastRounds`), 이미 보상 받은
  1회차 참가자에게 참가 뱃지 1회 채움, 끝난 회차 점수 기록 차단, `/event/hall` 신설.
- 앱: 회차 사이 대회 화면 = **대회 대기중(2회차 9/28~10/11 · D-day) + 명예의 전당**(시상대·4~10위·참가자).
  결과 팝업이 `2026-0901 회차` 대신 회차 번호 + 받은 뱃지를 보여준다.

### ④ 난이도별 성장 재설계 (09-15 · 사장님 확정)
- 목표: 극한까지 **90일**(14·21·25·30) · 최종 보스 때 채움 **50·65·75·85%** · 보스가 확실한 관문.
- 시뮬 보강: '현실적 최고치'(`tool/balance_targets.json`) · 장비는 공방 규칙으로 계산 · 난이도 이월 모델에
  골드·재료 초기화 반영 · 위협 계산에 회차 누락 수정 · `--fit-tiers`(난이도별 표) · 도착 직후 생존 표.
- 게임: `run_config.json → zoneTiers`(난이도별 체력·보스·위협·골드 표, `RunConfig.zoneTier`). 결과 91일.
- **버그 수정**: 위협 기준에 곤충 체력이 섞여 좋은 곤충을 낄수록 캐릭터가 크게 맞았다(한 대 51% → 15%).
- **서버**: 골드 상식 상한이 펫·장비 유저의 정당한 수입을 잘랐다 → 공격 ×100 으로 잰다(`clamp_check`).
- 후속(같은 날): 공방 최대 16·13레벨부터 ×15(극한 장비 176%→92%) · 보스 2.5초 템포(한 대 1.6배) ·
  사냥터 클리어 보상을 그 사냥터 30분치로(난이도별 기록) · 급소 노리기 % 소수 1자리 ·
  사냥터 보스 그림을 지역 반전(bossFlip)에서 제외(계곡·밤산 사냥터에서 등을 돌렸다) ·
  개발자 갤러리에 사냥터 보스 44마리 탭. 표 재산출 `--fit-days=14,22,27,35` → 14·21·26·30일.
- 후속2(같은 날): 공방 최대 19(화면 20등급, 끝 구간 ×1.6) · **난이도 이동 B안**(가 본 난이도까지 자유 이동, 처음 가는
  난이도만 초기화, 사냥터 전환은 난이도 유지) · 동기화·서버 허용치를 `topTier` 기준으로 · 보스 그림 8종 재절단·반전
  (e01·e03·e06·e07·e09·e_final·n02·x_final, 덩어리 단위 자르기).
- ⚠️ 펫·도감 축적은 가정 곡선 — 검증 필요.

### 배포 순서 (⚠️ 이 순서를 지킨다)
1. Supabase SQL: `_sql_20260915_rank_power.sql` → `_sql_20260915_chat_badge.sql` (대장 `sql_migrations.md`).
2. 서버 재배포 — `event.json` 2회차·pastRounds 가 서버에도 들어가야 한다. ⚠️ 서버에는 09-14
   사냥터 표·밸런스 변경도 함께 실린다(서버 방치 정산이 새 표를 쓴다) — 앱 출시와 간격을 짧게.
3. 앱 1.0.12(안드로이드 → iOS), 출시노트 양 스토어, `LATEST_VERSION_*`.

### 검증 (2026-09-15)
core_models 100 · core_battle 24 · core_run 279 · core_save 99 · server 313 · app 241 — 전부 통과.

### 남은 것
- 실기기 튜닝 마무리, `reachStage`("사냥터 N 도달") 미션 없음, 로드맵 챕터 `difficulty` 잔재. → `docs/design_zones.md`.
- 1회차 1위 실물 경품: 상위권 수동 검증(§4) → 신청 폼(`event.json → prizeFormUrl` 은 아직 비어 있음).
