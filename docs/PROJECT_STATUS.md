...
> 마지막 갱신: **2026-08-26** — §0(2026-08-17~25, **1.0.6 개발·출시 사이클**)을 추가했다.
> 로컬에만 있던 67커밋을 `origin/main` 에 푸시 완료(`efc85fe..984d7f9`).
>
...
## 0. 2026-08-17 ~ 08-25 — 1.0.6 사이클 (커밋 완료 · 푸시 완료)

`1.0.5` 이후 **41커밋**. 큰 축 넷: **왕충 선발대회(실물 경품)** · **결투 화면 전면 개편** ·
**광고 없는 운영으로 전환** · **젤리 소비처 무한화**. 마지막 커밋은 08-25 대회 1회차 연기다.

### ① 왕충 선발대회 (이벤트) — 실물 경품 랭킹전
- 웨이브 방어전 + 강화 카드 선택. **참가 곤충 스탯을 평준화**해 편성 실력으로 겨룬다
  (P2W 방지 원칙 §2.6 의 연장 — 키운 개체가 아니라 **오행 편성**이 순위를 가른다).
- 웨이브 = **단일 오행**이라 다음 적 속성을 보고 선봉을 바꾸는 게 핵심. 상극 표시·타격 모션 포함.
- 참가권은 **하루 2회 무료**(광고 없는 운영). 개발자 모드에 참가권 지급 스위치.
- **일정은 `event.json` 하나로 움직인다.** 1회차: 8/19~8/30 → 8/28~9/3(7일) → **9/1 00:00 ~
  9/15 00:00 KST(14일)** 로 두 번 연기. 코드 수정 없이 JSON 만 바꿨다(§6 규칙 그대로).
- ⚠️ **실물 경품은 법적 고지가 코드보다 먼저다** — 공식 규정·고지를 전단지에 명시
  (`docs/event_ranking_prize.md`). 국내 배송 한정도 스토어 노트에 적었다.

### ② 결투 화면 전면 개편
- 대각 전투 구도 + VS 인트로 · 히트스톱 · KO · 승패 배너, 로비 무대.
- ⚠️ **타격감이 없던 진짜 원인은 연출이 아니라 프레임이었다** — 전투 이벤트가 **초당 1.6프레임**으로
  흘러가고 있었다. 연출을 아무리 올려도 안 보이던 이유.
- ⚠️ `easeOutBack` 이 **0~1 범위를 넘는 값을 뱉어** `Opacity` 가 오류 화면을 띄웠다. 곡선을 쓰는 곳은
  값 범위를 보장하는 위젯인지 확인할 것.
- 아트: **곤충 20종 × 전투 자세 3종(대기·공격·피격) 60장** + 오행 아이콘 5장.
  기질·특성·리그·성별 아이콘도 전부 그림으로 교체(이모지 잔재 8군데 정리).
- 오행 관계도 범례 — "다음 적 속성"을 누르면 상성표가 열린다.

### ③ 광고 없는 운영으로 전환 (2026-08-18)
- 애드몹 **무효 트래픽 정지** 후 사장님 결정 → CLAUDE.md §2.6 확정사항으로 승격.
  광고 자리는 전부 **무료 지급 + 하루 상한**으로 바꿨다. `REAL_ADS=on` 으로만 복귀 가능(기본 off).
- 정지 전에 넣어둔 **무효 트래픽 방어**(쿨다운·기기 총량·테스트 기기 등록)는 코드에 남아 있다 —
  복귀할 때 `docs/ad_policy.md` 와 함께 쓴다. **새 애드몹 계정 재가입 금지.**
- `remove_ads` 상품 판매 중단(`hidden`) — 정의는 남긴다(기보유자 매일 젤리 10 지급이 참조).
- 잔재 정리: 부화기 무료 단축 제거, 버튼 글자 짤림 일괄 수정.

### ④ 젤리 소비처를 무한하게 (경제 정비)
- 08-15 에 **수도꼭지**를 잠갔더니(193→38/일) 이번엔 **하수구**가 문제였다 — 영구 소비처가
  390젤리뿐이라 중반 유저 기준 16일이면 말랐다. 방향이 반대인 같은 문제다.
- 처방 둘: **계단식 확장 비용**(사면 살수록 오른다) + **교환소**(젤리 → 골드·재료, 상시 소비).
- **무한 버프 패스(30일)** — 5종 버프 상시 발동. **광고 제거 패스는 삭제**(광고가 없어졌다).
- ⚠️ `jelly_sim` 이 **패스를 못 보던 구멍**을 메웠다 — 시뮬이 실제 경제와 어긋나 있었다.
- 깜짝선물 무료 2배: 하루 5회 → **하루 1회**(계속 받는 건 패스의 값어치). 소진 시 상점 유도.

### ⑤ 결제 사고 두 건 (⚠️ 재발 방지)
- **기간제 패스가 비소모품 경로로 나가 있었다** → 만료 후 연장 재구매가 **영영 불가**.
  consume 경로로 전환. 기간제는 소모품이다.
- **iOS `idle_pass` 가 ASC 에 비소모품으로 등록된 사고** → 상품 ID 는 지울 수 없으므로
  **iOS 전용 ID `idle_pass_c` 별칭**으로 우회. `docs/appstore_iap.md` 참조.

### ⑥ 그 외 수정
- **결투 시작이 전부 400** — 부상을 세션 확정 **전에** 선차감해 자기 선차감에 걸렸다. 확정 뒤로 옮김.
- **시즌 보상 기준을 최고 기록 → 종료 시 등급으로.** 최고 기록 기준이면 한 번 찍고 방치하는 게 이득이다.
- 세로 고정(가로 회전 시 레이아웃 붕괴), 편지함 시트가 상점 위에 남던 문제, 채집함 성별 표시,
  순위표 상위 3위 강조, 적 이름 현지화.
- 상점: 교환소 화면 + 머리그림 + 상점 배경.

### 검증 (2026-08-26, HEAD `984d7f9`)
전 패키지 테스트 **총 781 통과** — core_models 71 · core_battle 22 · core_run 143 ·
core_save 73 · server 277 · app 195.

### 남은 것
- **9/1 대회 1회차 오픈 준비** — 서버 반영·전단지·경품 고지 최종 점검(개막 전 예고 포함).
- 결제 실기 확인(패스 재구매·iOS `idle_pass_c`).
- ⚠️ Cloud Run `LATEST_VERSION_ANDROID` / `LATEST_VERSION_IOS` 가 프로덕션 반영본과 맞는지
  확인 — 저장소에서는 알 수 없다(`docs/release_cadence.md` §2).

---

## 0-2. 2026-08-15 세션 — 대규모 개편 (커밋 완료)
...
## 0-3. 2026-08-16 — 실기 확인 피드백 11건 (커밋 완료)
...
- **패키지 구조**(의존 방향 `app → core_battle/core_run → core_models`):
  ...
  - `app` — UI·저장·데이터·다국어.
- **검증 상태**(2026-08-26): core_models 71 · core_battle 22 · core_run 143 · core_save 73 · **server 277** · 앱 195 = **총 781 통과**, 전 패키지 analyze 무결.
- **세이브 스키마**: `kSaveSchemaVersion = 18` — 이후 추가 필드는 **버전을 올리지 않는 호환 필드**(§4).
- **출시 상태**: pubspec **1.0.6+20260820**. 1.0.6 은 20260812→20260813→20260814→20260820 으로
  여러 번 나갔다(대회 일정·광고 폐지·결제 수정). ⚠️ 어느 빌드가 프로덕션인지·`LATEST_VERSION_*` 값은
  **저장소가 아니라 콘솔/Cloud Run 에 있다** — 배포 전 반드시 확인(`docs/release_cadence.md` §2·§5).
  버전 게이트: 공용 min=20260810(1.0.4 이하 하드컷).
...
### ⚠️ 종(species)을 추가하기로 하는 날 — 먼저 볼 것

**지금은 계획이 없다**(2026-08-27 확인). 20종은 고정 전제로 짜여 있다 —
아트 60장·종 고유 패시브 20개·도감이 전부 그 위에 얹혀 있다. 그래서 아래
세 가지는 **버전이 아니라 "종을 추가할 때"** 가 트리거다. 종이 늘지 않는 한
도달할 수 없는 경로라 지금 고치지 않는다.

1. **서버를 앱보다 먼저 배포한다.** `species.json` 을 앱과 서버가 같이 읽는데
   (`server/game_config.dart`), 서버가 옛 파일이면 새 종으로 등록된 방어팀을
   못 찾는다.
2. **모르는 `speciesId` 에서 앱과 서버가 다르게 동작한다.**
   - 앱(`_defenderTeam`): 그 상대를 통째로 **스킵**한다(`return null`).
   - 서버(`_defenderToBattleBug`): 이름을 `'???'` 로 때우고 **그대로 싸운다**.
   - 앱 `GameData.species()` 는 아예 `throw StateError` 다(호출부 14곳) —
     세이브 로드가 아니라 **UI 런타임**에서 터져 세이브 방어 오버레이가 못 막는다.
3. **스탠스 폴백이 `attack` 고정**이다(`app.dart` `_defenderToBattleBug`).
   스탠스는 전투의 핵심 축(공/방/회 상성)이라, 이름이 `???` 로 보이는 것보다
   이쪽이 실질적인 문제다 — 조용히 **다른 전투**가 된다.

---

### 다음 작업 (1.0.7 — 순서대로)

1. **세이브 파서 방어** ⭐ **아직 안 됨** (2026-08-26 확인) — `MaterialKind.fromKey` 가 모르는 키에
   여전히 `ArgumentError` 를 던진다(`core_models/lib/src/enums.dart:225`,
   `core_save/lib/src/save_game.dart:1363`). 1.0.7 에서 **재료를 하나라도 추가하는 순간**
   그 세이브를 받은 1.0.6 앱이 죽는다. 새 기능(새 재료·enum)보다 **먼저** 넣을 것.
   같은 패턴의 `fromKey`/`values.firstWhere` 가 다른 enum 에도 있는지 함께 훑는다.
...
3. **9/1 대회 1회차 오픈** — `event.json` 일정 확정(9/1 00:00 ~ 9/15 00:00 KST). 서버 반영·
   개막 전 예고·경품 고지 최종 점검. 실물 경품이라 고지 누락이 곧 리스크다.
4. **결제 실기 확인** — 기간제 패스 만료 후 재구매(consume 경로), iOS `idle_pass_c` 별칭.
5. **펫·스킬 탭 완성** — 지금은 준비 중 패널(`_PetsPanel`/`_SkillsPanel` 코드는 보존,
   `character_screen.dart` 분기만 되돌리면 복구). 펫 슬롯 채집함→캐릭터 이동도 이때.
6. 캐릭터 씬 폴리시 — 하의 아트 반영 확인, 걷기/공격 스프라이트 추가 프레임.
...
- [ ] 위 "다음 작업(1.0.7)" 순서대로 — ① **파서 방어(아직 안 됨)** ② 9/1 대회 오픈 ③ 결제 실기
