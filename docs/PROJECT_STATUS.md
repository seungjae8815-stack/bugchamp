...
| **랭킹/백엔드(Phase 4 Inc.1)** | `PvpBackend` 인터페이스 + `LocalPvpBackend`(로컬 사다리) + **랭킹 화면**(홈 랭킹 아이콘 진입). Supabase는 어댑터만 꽂으면 됨 | `domain/pvp_backend.dart`, `features/leaderboard/leaderboard_screen.dart`, `docs/backend_supabase.md` |
...
- 채팅 바(placeholder).
- **랭킹** → ✅ 화면 완료(로컬 사다리). **Supabase 실연동만 남음**(Phase 4 Inc.2: 방어팀 등록·스카우트 실데이터·인증). 키 필요 → `docs/backend_supabase.md`.
