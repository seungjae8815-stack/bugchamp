...
아스트레일이 보내기만 한다면 문제없다.
> ⚠️ **2026-09-15 사고:** 아스트레일 봇(@astrailcount_bot)을 **UGC 서버가 `getUpdates` 롱폴링으로 받고 있었다**
> (쓰레드 댓글 답글 승인 버튼). 웹훅을 거는 순간 텔레그램이 getUpdates 를 409 로 거부해 UGC 승인이 멈췄다.
> → 곤충키우기 문의는 **전용 봇 @bugchamp_bot** 으로 옮겼다(Cloud Run `TELEGRAM_BOT_TOKEN` 교체 + 새 봇에 웹훅).
> 아스트레일 봇은 웹훅 없이 UGC·일일 리포트가 그대로 쓴다. **아스트레일 봇에 다시 웹훅을 걸지 말 것.** 모르겠으면 **곤충키우기 전용 봇**을 새로 만든다
