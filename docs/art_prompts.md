...
> **2026-08-17 완료**: 기질 5 · 혈통 특성 4 · 리그 뱃지 5 · 성별 2 를 받아 넣었다
> (`ui/temperament` `ui/trait` `ui/league` `ui/sex`). 남은 건 전투 장소 5 와 상태 4.
>
> ⚠️ 생성기가 **리그 뱃지에 글자 띠를 그려 넣었다**(프롬프트에 `no text` 를 넣었는데도).
> 실제로 쓰는 16~24px 에서는 무늬로 뭉개져 안 보이지만, 크게 쓸 자리가 생기면
> 다시 뽑아야 한다.
>
> ⚠️ 성별 그림은 **체크무늬가 아니라 흰 배경**으로 나왔다. 그래서 누끼 도구에
> "민무늬 배경" 갈래를 넣었다 — 두 톤이 거의 같으면 반반 판정이 성립하지 않으므로,
> 갇힌 덩어리 중 **넓은 것만** 지운다(작은 건 그림 속 흰 하이라이트다).
