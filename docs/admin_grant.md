...
**예) 젤리를 1,500 으로 만들려면 `500` 을 입력한다.**

## 값 정하기 — 근거

- **젤리 1,500**: 무과금 최대 수입(고인물 45/일) x 출시 후 31일 ≈ 1,395.
  "오늘까지 정상적으로 모을 수 있는 최대"다. 알 뽑기 50회 + 영구 확장 일부가
  남으므로 벌이 아니라 정상화로 읽힌다. → 입력 **500**
- **재료 10억**: 스테이지 1000 의 재료 수입이 약 140만/시간 → **약 한 달치
  논스톱 파밍**. 정상 플레이로는 평생 못 닿는다. → 입력 **980000000**
