...
- [ ] **§8 전체 채팅** — 테이블·RLS·도배방지 트리거·신고 테이블 전부 실행
- [ ] **Database → Replication** 에서 `chat_messages` 를 `supabase_realtime` 에 추가
      (안 하면 실시간 수신이 조용히 멈춘다)
...
3. **닉네임 필터가 아직 없다.** 채팅 본문은 걸러지지만 닉네임은 무필터라
   부적절한 이름이 랭킹·채팅에 그대로 노출된다. 후속 과제.
4. **일본어 번역이 미완성**이다. 일본 대상 출시는 미루거나 영어 폴백으로 나간다.
5. 첫 출시는 **한국만** 대상으로 좁게 시작하는 편이 안전하다
   (문제 발견 시 영향 범위가 작다).
