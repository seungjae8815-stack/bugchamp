...
| `_sql_20260915_ops_monitor.sql` | 운영 감시 — 리포트 크론 재등록 · `ops_heartbeat` · `ops_settings`(텔레그램 확인 버튼 대기·채팅 요약) · ops-watchdog 10분 크론. ⚠️ **서버 재배포보다 먼저** (없으면 확인 버튼·채팅 요약이 동작하지 않는다) | 대기 |
