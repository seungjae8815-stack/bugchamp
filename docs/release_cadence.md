...
- iOS 는 직전 출시본이 오래됐을 수 있으니, 노트에 **그 사이 안드로이드에
  나간 내용까지 합쳐서** 적는다.
### ⚠️ iOS: 빌드번호만 올려도 되나? — **출시 전에는 된다**

| 그 버전(예 1.0.9)의 상태 | 빌드번호만 올리기 | 해야 할 일 |
|---|---|---|
| 아직 제출 안 함 / **심사 중** / 심사 거절 | **된다** | 빌드번호 +1 |
| **승인 + 출시 완료** | **안 된다** | 버전명을 올린다(1.1.0 등) |

한 번 출시되면 그 버전의 **트레인이 닫힌다**:
```
90062  CFBundleShortVersionString [1.0.8] must be higher than
       the previously approved version [1.0.8]
90186  Invalid Pre-Release Train — '1.0.8' is closed
```

실제로 우리가 양쪽을 다 겪었다:
- **1.0.6**: 심사 중이라 `20260829 → 20260830` 빌드번호만 올려 재제출 — **통과**.
- **1.0.7 · 1.0.9**: 앞 버전이 이미 출시돼 트레인이 닫힘 — **거부**.

안드로이드는 `versionCode` 만 유일하면 같은 버전명을 다시 써도 되므로
**여기서 두 스토어가 갈린다.** 갈린 채로 두면 "안드 1.0.8 = iOS 1.0.9 = 같은
내용"을 계속 기억해야 하므로, iOS 를 올릴 때 안드로이드도 같이 올려 맞춘다.

> 💡 요령: iOS 를 **심사 중일 때 고치면** 빌드번호만 올리면 된다. 출시 버튼을
> 누른 뒤에 고칠 것이 나오면 버전명이 하나 올라간다.
