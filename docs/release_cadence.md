...
- iOS 는 직전 출시본이 오래됐을 수 있으니, 노트에 **그 사이 안드로이드에
  나간 내용까지 합쳐서** 적는다.
- ⚠️ **iOS 는 버전 문자열(1.0.6)을 재사용할 수 없다.** 그 버전이 한 번
  승인되면 트레인이 닫혀, 빌드번호만 올려도 업로드가 거부된다
  (`90062 CFBundleShortVersionString must be higher` +
  `90186 train version is closed`). 안드로이드는 versionCode 만 유일하면
  같은 버전명을 다시 써도 되므로 **여기서 두 스토어가 갈린다.**
  → iOS 를 한 번 낸 버전에 무언가를 더 내보내려면 `1.0.7` 로 올린다.
  안드로이드도 같이 올려 두 스토어 버전을 맞추는 편이 나중에 덜 헷갈린다
  (2026-09-01, 1.0.6 재사용 시도로 겪음).
