...
| 7 | `idle_pass`        | **소모성** ⚠️ | 곤충학자 패스 30일 | 9,900 |
...
> (`store_iap_service.dart`). **이미 비소모성으로 만들어 버렸다면 유형 변경이
> 불가능하므로** 그 ID 는 버리고 새 ID(예: `idle_pass_c`)로 다시 만들고
> iap.json 의 id 도 맞춰야 한다.
>
