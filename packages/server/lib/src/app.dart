...
Handler buildHandler({
  ...
  if (gameConfig != null && speciesById != null) {
    ...
    Future<SaveGame?> loadSave(String uid) async {
      ...
      // ⚠️ 사냥터 세대 리셋(applyZoneEpoch)은 **서버에서 하지 않는다.**
      // 2026-09-14 여기서 돌렸다가 구버전 앱을 쓰는 실유저의 진행도가
      // 동기화 한 번에 1 로 떨어졌다(준범 st 978 → 1). 리셋은 새 앱이 자기
      // 로드에서만 하고, 서버는 그 결과를 받는다.
    ...
    /// 지급 우편의 제목·본문 — **상품 이름을 제목에 넣는다**.
    ///
    /// 고정 문자열("운영자 지급")만 쓰면 무엇을 받는지가 안 보인다. 유저는
    /// 자기가 무엇을 받는지로 우편을 판단한다(2026-09-01 지적).
    ///
    /// ⚠️ 이 우편은 **오직 운영 지급**(`/admin/grant`)에서만 나온다 —
    /// 실제 상점 결제는 우편을 거치지 않고 앱이 즉시 지급한다
    /// (`SaveController.applyPurchase`). 그래서 본문에 "상점 구입분"이라고
    /// 쓰면 거짓말이고, 반대로 **누가 줬는지를 밝혀야** 유저가 "운영자가
    /// 보냈구나"로 읽는다(2026-09-01 지시).
    String grantMailTitle(String productName) => '[운영자 지급] $productName';
    const grantMailBody = '운영자가 보낸 보상입니다. 받기를 눌러 수령하세요.';
