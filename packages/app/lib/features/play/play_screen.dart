...
class _PlayScreenState extends ConsumerState<PlayScreen>
  ...
  /// **지금 할 일** 한 줄. 흩어진 알림을 한곳에 모은다.
  ///
  /// 예전엔 부화 완료·부상 곤충·꺼진 버프·받을 보상이 전부 **다른 화면 안쪽**에
  /// 있어서, 유저가 탭을 하나씩 열어 봐야 알 수 있었다(2026-09-09 지적).
  /// 그 중 몇 개는 **젤리로 바로 해결되는 일**이라, 안 보이면 쓸 일도 없다.
  ///
  /// 규칙 두 가지:
  /// - 할 일이 없으면 **줄 자체가 사라진다**. 늘 떠 있으면 배경이 되어 안 읽힌다.
  /// - 각 배지는 **누르면 그 자리로 간다**. 알리기만 하고 길을 안 알려주면
  ///   결국 또 뒤져야 한다.
  Widget _todoBar(AppLocalizations l, SaveGame save) {
    final now = _clock.now().toUtc();
    final cfg = _data.petConfig;
    final items =
        <({IconData icon, String label, Color color, VoidCallback go})>[];

    // 부화 완료 — 무료지만 방치되기 쉽다(수령해야 다음 알이 들어간다).
    if (cfg != null && save.incubating.values.any((t) => !t.isAfter(now))) {
      items.add((
        icon: Icons.egg_alt,
        label: l.todoHatch,
        color: const Color(0xFF66BB6A),
        go: () => ref.read(tabIndexProvider.notifier).set(2),
      ));
    }
    // 짝짓기 완료.
    if (save.breeding.any((b) => !b.endsAt.isAfter(now))) {
      items.add((
        icon: Icons.favorite,
        label: l.todoBreed,
    ...
    // 부상 곤충 — **젤리로 즉시 회복**하는 자리다.
    final hurt = save.injured.values.where((t) => t.isAfter(now)).length;
    if (hurt > 0) {
      items.add((
        icon: Icons.healing,
        label: l.todoInjured(hurt),
        color: const Color(0xFFFFB74D),
        go: () => ref.read(tabIndexProvider.notifier).set(3),
      ));
    }
    // 꺼진 버프 — 우측 세로 5칸은 맥동해도 눈에 안 띈다.
    if (BuffKind.values.any((k) => save.buffRemaining(k, now) == null)) {
      items.add((
        icon: Icons.bolt,
        label: l.todoBuffOff,
        color: _honey,
    ...
    // 채집함 가득 — 두면 곤충 획득이 통째로 막힌다(§2.1).
    if (save.storageFull) {
      items.add((
        icon: Icons.inventory_2,
        label: l.todoStorageFull,
        color: const Color(0xFFEF5350),
        go: () => ref.read(tabIndexProvider.notifier).set(2),
      ));
    }
    // 결투 티켓이 찼다 — 안 쓰면 자연 충전이 멈춘다(상한 10).
    final tickets = save.pvpTickets;
    if (tickets >= 10) {
      items.add((
        icon: Icons.sports_kabaddi,
        label: l.todoTickets(tickets),
    ...
    // 맥동은 **줄 전체**가 아니라 테두리만 — 글자가 깜빡이면 읽기 어렵다.
    final pulse = 0.5 + 0.5 * math.sin(_tapHint * 2.4);
    return SizedBox(
      height: 26,
        ...
        itemBuilder: (_, i) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              alignment: Alignment.center,
              decoration: BoxDecoration(
                color: it.color.withValues(alpha: 0.20),
                borderRadius: BorderRadius.circular(999),
                border: Border.all(
                  color: it.color.withValues(alpha: 0.55 + pulse * 0.45),
                  width: 1.2,
                ),
              ),
              child: Row(
        ...
        child: Transform.translate(
          ...
          child: Stack(
            children: [
              ...
              // 지금 할 일 — 버프 줄 **아래**, 가로 한 줄. 할 일이 없으면 사라진다.
              // 왼쪽 퀘스트 칸(폭 120)과 오른쪽 버프 칸을 피해 가운데를 쓴다.
              Positioned(
                left: 132,
                right: 40,
                top: 12,
